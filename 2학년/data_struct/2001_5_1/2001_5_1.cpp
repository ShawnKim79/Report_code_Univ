// 역시 링크드리스트...단...리스트의 값 평균도 내주는 리스트
...
void list_average()
{
	list_pointer c;                     // list_pointer c 선언
	float av = 0.0;                     // 평균값 저장
	int n = 1;                          // 노드갯수 카운트
	cout << endl << " * average of linked-list : ";

	if(!ptr){                           // ptr이 NULL이 아니라면
		cout << " list is empty \n";    // List는 비어있는 상태.
		return;
	}
	av += float(ptr->data);             // av에 ptr의 데이터 저장.
	c=ptr->link;                        // c의 주소는 ptr의 링크.
	while(c){                           // c의 주소가 존재할 동안
		n++;                            // 키운트 증가.
		av += float(c->data);
		c=c->link;                      // c의 주소는 c의 링크 주소 
		                                // : 즉 다음 노드의 주소를 갖게된다.
	} 
	av = av / float(n);                 // 평균값 계산.
	cout << av << endl;
}
